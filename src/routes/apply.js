...
router.post('/start', requireSession, upload.array('documents', 6), async (req, res) => {
  try {
    ...
    // Force session save and redirect to interview page
    return new Promise((resolve, reject) => {
      req.session.save((err) => {
        ...
        console.log('Session saved successfully in /start, redirecting to interview:', req.session);
        res.redirect('/apply/interview');
        resolve();
      });
    });
  } catch (err) {
